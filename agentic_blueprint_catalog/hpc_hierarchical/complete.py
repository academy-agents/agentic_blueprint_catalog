""" This example uses GlobusCompute on Aurora to launch Director agents on the lead node
of a 4 node batch job. The Directors partition the batch job into even splits of 2 nodes each.
Each Director uses Parsl to run a mock md_sim_tool to tasks on.
"""
...
async def main():
    ...
    async with await Manager.from_exchange_factory(
        ...
        d1_task = asyncio.create_task(director1_handle.md_sim_batch(iterations=4))
        d2_task = asyncio.create_task(director2_handle.md_sim_batch(iterations=4))
