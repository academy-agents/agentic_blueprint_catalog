...
class Director(Agent):
        ...
        config = Config(
            executors=[
                HighThroughputExecutor(
                    ...
                    # Use local provider since we are running parsl inside provisioned batch job
    ...
    async def md_sim(self):
        """md_sim is a blocking call executed by Parsl over the compute nodes"""
        return await asyncio.wrap_future(md_sim_tool())

    @action
    async def md_sim_batch(self, iterations: int = 4):
        """Submit a batch of MD simulation calls executed in parallel with Parsl"""
